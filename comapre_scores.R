...
source(file.path("risk_scores/array_job/asx_strat_cent",'ARS_utils.R'), echo = T)
source(file.path("risk_scores/array_job/centered",'res_utils.R'), echo = T)

source(file.path("risk_scores/array_job/centered",'get_metrics.R'), echo = T)
...
#results paths
store_asx <- paste0("/ukb_models/cut_off/cv/stacked/asx/center/asx_",asx_type,'_',asx_suffix)
store_asx_pa <- paste0("/ukb_models/cut_off/cv/stacked/asx/center/asx_",'pa','_','lin')

store_asx_old <- paste0("/ukb_models/cut_off/cv/stacked/asx/asx_",asx_type,'_',asx_suffix)
store_ars <- paste0('/',data_source,"_models/cut_off/cv/stacked/all_nri/center/",model,"_ars_fp")
store_ars_old <- paste0('/',data_source,"_models/cut_off/cv/stacked/all_nri/",model,"_ars_fp")
store_pa <- paste0('/',data_source,"_models/cut_off/cv/stacked/all_nri/center/",model,"_pa_lin")
...
df_asx <- get_predictions(
  ...
  store = "/ukb_models/cut_off/cv/stacked/asx/center",
  model = "asx",
  split = "",
  model_type = asx_type,
...
df_pa_only <- get_predictions(
  env_path = "risk_scores/stacked/array_job",
  data_path = "risk_scores/ARS/bs_imp/con/6y/stacked/",
  store = "/ukb_models/cut_off/cv/stacked/pa_only/center",
  model = "acc",
  split = "",
  model_type = "pa",
  store_suffix = "lin",
...
df_pa <- get_predictions(
  env_path = "risk_scores/stacked/array_job",
  data_path = "risk_scores/ARS/bs_imp/con/6y/stacked/",
  store = paste0("/",data_source,"_models/cut_off/cv/stacked/all_nri/center"),
  model =model,
  split = "",
  model_type = "pa",
  store_suffix = "lin",
...
if(met == 'c_idx'){
  ...
  args_list2  <- list(lienar_predictions_pa[[1]][[2]],sex_cond)
}else if (met == 'ici'){
...
if(met == 'c_idx'){
  ...
  args_list1 <-   list(df = df_pa, sex_cond = sex_cond,rs1 = 'qrisk3',rs2 = 'ars')
  args_list2 <-   list(df = df_ars, sex_cond = sex_cond,rs1 = 'qrisk3',rs2 = 'ars')
}
...
if(met == 'c_idx'){
  ...
  args_list1 <-   list(df = df_ars, sex_cond = !sex_cond,rs1 = 'qrisk3',rs2 = 'ars')
}
