source(file.path('ARS_eval_utils_try_catch.R'), echo = T)
source(file.path('reclass.R'), echo = T)
library(tidyr)
...
load(paste0(split,"/qrisk_pa_lin/ukb_qrisk_pa.RData"))
...
performance_shrinkage <-function(mods,rows_with_na,met,mode = 'mean'){
 ...
 if(mode == 'mean'){
    ...
    male_q_us =  sd(male_q_bs_cbl - male_q_cbl)
    male_ars_us =  sd(male_ars_bs_cbl - male_ars_cbl)
    female_q_us =  sd(female_q_bs_cbl - female_q_cbl)
    female_ars_us =  sd(female_ars_bs_cbl - female_ars_cbl)
  }
...
adjust_lp <- function(imputed_datasets,mods,shrinkage,model = 'qrisk',fp_terms = NULL){
  ...
  for(i in 1:5){
    print("q m")
    q_imp_lp_m[[i]] <-shrinkage[[1]] * get_lp(imputed_datasets[[i]][imputed_datasets[[i]]$Sex ==1,],mods[[1]],ars = FALSE,model = model,fp_terms = fp_terms)
    print("ars m")
    ...
    print("q f")
    q_imp_lp_f[[i]] <- shrinkage[[3]] * get_lp(imputed_datasets[[i]][imputed_datasets[[i]]$Sex ==0,],mods[[3]],ars = FALSE,model = model,fp_terms = fp_terms)
...
load(paste0("risk_scores/stacked/array_job/ukb_models/cut_off/cv/split",split,"/qrisk_pa_lin/ukb_qrisk_ars.RData"))
...
c_shrink <- performance_shrinkage(full_res,rows_with_na,met = 'c_idx')
ici_shrink <- performance_shrinkage(full_res,rows_with_na,met = 'ici')
r2_shrink <- performance_shrinkage(full_res,rows_with_na,met = 'r2')
nb_10_shrink <- performance_shrinkage(full_res,rows_with_na,met = 'nb_10')
nb_7_shrink <- performance_shrinkage(full_res,rows_with_na,met = 'nb_7.5')
nb_5_shrink <- performance_shrinkage(full_res,rows_with_na,met = 'nb_5')

#SD optim
c_sd <- performance_shrinkage(full_res,rows_with_na,met = 'c_idx',mode = 'sd')
ici_sd <- performance_shrinkage(full_res,rows_with_na,met = 'ici',mode = 'sd')
r2_sd <- performance_shrinkage(full_res,rows_with_na,met = 'r2',mode = 'sd')
nb_10_sd <- performance_shrinkage(full_res,rows_with_na,met = 'nb_10',mode = 'sd')
nb_7_sd <- performance_shrinkage(full_res,rows_with_na,met = 'nb_7.5',mode = 'sd')
nb_5_sd <- performance_shrinkage(full_res,rows_with_na,met = 'nb_5',mode = 'sd')

#get condensed model rows
rows_with_na_con <- null_rows(cond_res)
shrinkage_con <- get_optimism(cond_res,rows_with_na_con)
c_shrink_con <- performance_shrinkage(cond_res,rows_with_na_con,met = 'c_idx')
ici_shrink_con <- performance_shrinkage(cond_res,rows_with_na_con,met = 'ici')
r2_shrink_con  <- performance_shrinkage(cond_res,rows_with_na_con,met = 'r2')
nb_10_shrink_con <- performance_shrinkage(cond_res,rows_with_na_con,met = 'nb_10')
nb_7_shrink_con <- performance_shrinkage(cond_res,rows_with_na_con,met = 'nb_10')
nb_5_shrink_con <- performance_shrinkage(cond_res,rows_with_na_con,met = 'nb_10')


#SD optim
c_sd_con <- performance_shrinkage(cond_res,rows_with_na_con,met = 'c_idx',mode = 'sd')
ici_sd_con  <- performance_shrinkage(cond_res,rows_with_na_con,met = 'ici',mode = 'sd')
r2_sd_con  <- performance_shrinkage(cond_res,rows_with_na_con,met = 'r2',mode = 'sd')
nb_10_sd_con  <- performance_shrinkage(cond_res,rows_with_na_con,met = 'nb_10',mode = 'sd')
nb_7_sd_con  <- performance_shrinkage(cond_res,rows_with_na_con,met = 'nb_7.5',mode = 'sd')
nb_5_sd_con  <- performance_shrinkage(cond_res,rows_with_na_con,met = 'nb_5',mode = 'sd')
...
lp_full <- adjust_lp(imputed_datasets,full_models,c(1,1,1,1),model = 'qrisk',fp_terms = fp_terms)#shrinkage
lp_con <- adjust_lp(imputed_datasets,cond_models,c(1,1,1,1),model ='qrisk',fp_terms = fp_terms)#shrinkage_con

lp_adj_full <- adjust_lp(imputed_datasets,full_models,shrinkage,model = 'qrisk',fp_terms = fp_terms)#shrinkage
lp_adj_con <- adjust_lp(imputed_datasets,cond_models,shrinkage_con,model ='qrisk',fp_terms = fp_terms)#shrinkage_con

print('male pooled S0')
sex_cond <- df$Sex == 1
#lapply(q_bs_mod_m,function(mod) {get_base_surv_mp(df_bs[sex_cond,],time = 6,mod =mod)})
...
base_male_q_con <- lapply(q_imp_condensed_m,function(mod) {get_base_surv_mp(df[sex_cond,],time = 6,mod =mod,)})#lp = lp_adj_con[[1]]
...
rs_full <- adjust_rs(imputed_datasets,full_models,lp_full,base_surv = baseline_surv)
...
###########Record optimism adjusted results 
get_nbs <- function(df,rs,cond,shrink,shrink_idx){
  nb_10 <- calc_nb(df[cond,],df[cond,rs],lab = 'macce.incident',th = 0.1) - shrink[[1]][[shrink_idx]]
  nb_7_5 <- calc_nb(df[cond,],df[cond,rs],lab = 'macce.incident',th = 0.075) - shrink[[2]][[shrink_idx]]
  nb_5 <- calc_nb(df[cond,],df[cond,rs],lab = 'macce.incident',th = 0.05) - shrink[[3]][[shrink_idx]]
...
# C-index
c_adj_q_m <- get_idx(df[sex_cond,], Reduce(`+`, lp_adj_full[[1]]) / 5) - c_shrink[[1]]
c_adj_ars_m <- get_idx(df[sex_cond,], Reduce(`+`, lp_adj_full[[2]]) / 5) - c_shrink[[2]]
c_adj_q_con_m <- get_idx(df[sex_cond,], Reduce(`+`, lp_adj_con[[1]]) / 5) - c_shrink_con[[1]]
c_adj_ars_con_m <- get_idx(df[sex_cond,], Reduce(`+`, lp_adj_con[[2]]) / 5) - c_shrink_con[[2]]

#ICI
ici_adj_q_m <- get_cal(df[sex_cond,],df[sex_cond,'qrisk3'],lab = 'macce.incident') - ici_shrink[[1]]
ici_adj_ars_m <-get_cal(df[sex_cond,],df[sex_cond,'ars'],lab = 'macce.incident') - ici_shrink[[2]]
ici_adj_q_con_m <- get_cal(df[sex_cond,],df[sex_cond,'qrisk3_con'],lab = 'macce.incident') - ici_shrink_con[[1]]
ici_adj_ars_con_m <- get_cal(df[sex_cond,],df[sex_cond,'ars_con'],lab = 'macce.incident') - ici_shrink_con[[2]]

#r2 
r2_adj_q_m <- get_r2( Reduce(`+`, lp_full[[1]]) / 5) - r2_shrink[[1]]
r2_adj_ars_m <- get_r2(Reduce(`+`, lp_full[[2]]) / 5) - r2_shrink[[2]]
r2_adj_q_con_m <- get_r2( Reduce(`+`, lp_con[[1]]) / 5) - r2_shrink_con[[1]]
r2_adj_ars_con_m <- get_r2( Reduce(`+`, lp_con[[2]]) / 5) - r2_shrink_con[[2]]

#NB
nb_adj_q_m <- get_nbs(df,'qrisk3',sex_cond,list(nb_10_shrink,nb_7_shrink,nb_5_shrink),1)
nb_adj_ars_m <- get_nbs(df,'ars',sex_cond,list(nb_10_shrink,nb_7_shrink,nb_5_shrink),2)
nb_adj_q_con_m <- get_nbs(df,'qrisk3_con',sex_cond,list(nb_10_shrink_con,nb_7_shrink_con,nb_5_shrink_con),1)
nb_adj_ars_con_m <- get_nbs(df,'ars_con',sex_cond,list(nb_10_shrink_con,nb_7_shrink_con,nb_5_shrink_con),2)

#######
#Female
########
# C-index
c_adj_q_f <- get_idx(df[!sex_cond,], Reduce(`+`, lp_adj_full[[3]]) / 5) - c_shrink[[3]]
c_adj_ars_f <- get_idx(df[!sex_cond,], Reduce(`+`, lp_adj_full[[4]]) / 5) - c_shrink[[4]]
c_adj_q_con_f <- get_idx(df[!sex_cond,], Reduce(`+`, lp_adj_con[[3]]) / 5) - c_shrink_con[[3]]
c_adj_ars_con_f <- get_idx(df[!sex_cond,], Reduce(`+`, lp_adj_con[[4]]) / 5) - c_shrink_con[[4]]

#ICI
ici_adj_q_f<- get_cal(df[!sex_cond,],df[!sex_cond,'qrisk3'],lab = 'macce.incident') - ici_shrink[[3]]
ici_adj_ars_f <-get_cal(df[!sex_cond,],df[!sex_cond,'ars'],lab = 'macce.incident') - ici_shrink[[4]]
ici_adj_q_con_f <- get_cal(df[!sex_cond,],df[!sex_cond,'qrisk3_con'],lab = 'macce.incident') - ici_shrink_con[[3]]
ici_adj_ars_con_f <- get_cal(df[!sex_cond,],df[!sex_cond,'ars_con'],lab = 'macce.incident') - ici_shrink_con[[4]]

#r2 
r2_adj_q_f <- get_r2( Reduce(`+`, lp_full[[3]]) / 5) - r2_shrink[[3]]
r2_adj_ars_f <- get_r2(Reduce(`+`, lp_full[[4]]) / 5) - r2_shrink[[4]]
r2_adj_q_con_f <- get_r2( Reduce(`+`, lp_con[[3]]) / 5) - r2_shrink_con[[3]]
r2_adj_ars_con_f <- get_r2( Reduce(`+`, lp_con[[4]]) / 5) - r2_shrink_con[[4]]

#NB
nb_adj_q_f <- get_nbs(df,'qrisk3',!sex_cond,list(nb_10_shrink,nb_7_shrink,nb_5_shrink),3)
nb_adj_ars_f <- get_nbs(df,'ars',!sex_cond,list(nb_10_shrink,nb_7_shrink,nb_5_shrink),4)
nb_adj_q_con_f <- get_nbs(df,'qrisk3_con',!sex_cond,list(nb_10_shrink_con,nb_7_shrink_con,nb_5_shrink_con),3)
nb_adj_ars_con_f <- get_nbs(df,'ars_con',!sex_cond,list(nb_10_shrink_con,nb_7_shrink_con,nb_5_shrink_con),4)

save.image(file=paste0('risk_scores/ARS/ukb_ars_fp_split',split,'.RData'))
#########################################################
...
df['qrisk3_prob'] <-df$qrisk3_adj/100
df['ars_prob'] <-df$ars_adj/100

df['qrisk3_con_prob'] <-df$qrisk3_con_adj/100
df['ars_con_prob'] <-df$ars_con_adj/100

#full
store_path <- "risk_scores/array_job/master_branch/plot_scripts/outputs/ukb_plots/nb_curve/full/"
png(paste0(store_path,"female_pa_nb.png" ),type="cairo")  
pgrs_nb <- stdca(data=df[!sex_cond,], outcome="macce.incident", ttoutcome="macce_obs_time", timepoint=6.0, 
                 predictors=preds, xstop=0.2,ymin = -0.02, loess.span=0.2, smooth=TRUE)

dev.off()

#con
preds <-c('qrisk3_prob','qrisk3_con_prob','ars_con_prob')
store_path <- "risk_scores/array_job/master_branch/plot_scripts/outputs/ukb_plots/nb_curve/con/"
png(paste0(store_path,"ovr_pa_con_nb.png" ),type="cairo")  
pgrs_nb <- stdca(data=df[,], outcome="macce.incident", ttoutcome="macce_obs_time", timepoint=6.0, 
                 predictors=preds, xstop=0.2,ymin = -0.02, loess.span=0.2, smooth=TRUE)

dev.off()

#all
preds <-c('qrisk3_prob','ars_prob','qrisk3_con_prob','ars_con_prob')
store_path <- "risk_scores/array_job/master_branch/plot_scripts/outputs/ukb_plots/nb_curve/all/"
png(paste0(store_path,"ovr_pa_nb_all.png" ),type="cairo")  
pgrs_nb <- stdca(data=df[,], outcome="macce.incident", ttoutcome="macce_obs_time", timepoint=6.0, 
                 predictors=preds, xstop=0.2,ymin = -0.02, loess.span=0.2, smooth=TRUE)
...
calc_nb(df[sex_cond,],df[sex_cond,'qrisk3'],lab = 'macce.incident',th = th) - nb_10_shrink[[1]]
(calc_nb(df[sex_cond,],df[sex_cond,'qrisk3'],lab = 'macce.incident',th = th) - nb_10_shrink[[1]]) + c(-2, 0, 2) * nb_10_sd[[1]] / sqrt(nrow(df))

calc_nb(df[sex_cond,],df[sex_cond,'ars'],lab = 'macce.incident',th = th) -  nb_10_shrink[[2]]
(calc_nb(df[sex_cond,],df[sex_cond,'ars'],lab = 'macce.incident',th = th) - nb_10_shrink[[2]]) + c(-2, 0, 2) * nb_10_sd[[2]] / sqrt(nrow(df))

calc_nb(df[!sex_cond,],df[!sex_cond,'qrisk3'],lab = 'macce.incident',th = th) -  nb_10_shrink[[3]]

calc_nb(df[!sex_cond,],df[!sex_cond,'ars'],lab = 'macce.incident',th = th) - nb_10_shrink[[4]]
...
age_cond1 <- df$age <65
age_cond2 <- df$age >=65 & df$age<75
age_cond3 <- df$age >=65

nri_cond <- rep(TRUE,nrow(df))#rep(TRUE,nrow(df)) #age_cond3
th <- 0.1
get_nri(df[sex_cond&nri_cond ,],'qrisk3_adj','ars_adj','macce.incident',cutoff = c(0,th,1))
get_nri(df[!sex_cond&nri_cond,],'qrisk3_adj','ars_adj','macce.incident',cutoff = c(0,th,1))
get_nri(df[,],'qrisk3_adj','ars_adj','macce.incident',cutoff = c(0,th,1))
...
rename_col <- function(res, name, new) {
  for (i in seq_along(res)) {
    for (j in seq_along(res[[i]])) {
      for (k in seq_along(res[[i]][[j]])) {
        if (name %in% colnames(res[[i]][[j]][[k]])) {
          colnames(res[[i]][[j]][[k]])[colnames(res[[i]][[j]][[k]]) == name] <- new
        }
      }
    }
  }
...
#c-index by age
full_res <- rename_col(full_res,'c_under_65_q','c_under_65')
full_res <- rename_col(full_res,'c_under_65_ars','c_under_65')
c_shrink_65 <- performance_shrinkage(full_res,rows_with_na,met = 'c_under_65')
full_res <- rename_col(full_res,'c_65_75_q','c_65_75')
full_res <- rename_col(full_res,'c_65_75_ars','c_65_75')
c_shrink_65_75 <- performance_shrinkage(full_res,rows_with_na,met = 'c_65_75')
full_res <- rename_col(full_res,'c_over_75_q','c_over_75')
full_res <- rename_col(full_res,'c_over_75_ars','c_over_75')
c_shrink_75 <- performance_shrinkage(full_res,rows_with_na,met = 'c_over_75')
male_q = full_res[[1]][[1]]
male_ars = full_res[[1]][[2]]
female_q = full_res[[1]][[3]]
female_ars = full_res[[1]][[4]]
...
m_c_65 <- c_ci_change(combine_imp(male_ars,'c_under_65'),combine_imp(male_q,'c_under_65'),c_shrink_65[[2]],c_shrink_65[[1]])
m_c_65_75 <- c_ci_change(combine_imp(male_ars,'c_65_75'),combine_imp(male_q,'c_65_75'),c_shrink_65_75[[2]],c_shrink_65_75[[1]])
m_c_75 <- c_ci_change(combine_imp(male_ars,'c_over_75'),combine_imp(male_q,'c_over_75'),c_shrink_75[[2]],c_shrink_75[[1]])
m_c <- c_ci_change(combine_imp(male_ars,'c_idx'),combine_imp(male_q,'c_idx'),c_shrink[[2]],c_shrink[[1]])

f_c_65 <- c_ci_change(combine_imp(female_ars,'c_under_65'),combine_imp(female_q,'c_under_65'),c_shrink_65[[4]],c_shrink_65[[3]])
f_c_65_75 <- c_ci_change(combine_imp(female_ars,'c_65_75'),combine_imp(female_q,'c_65_75'),c_shrink_65_75[[4]],c_shrink_65_75[[3]])
f_c_75 <- c_ci_change(combine_imp(female_ars,'c_over_75'),combine_imp(female_q,'c_over_75'),c_shrink_75[[4]],c_shrink_75[[3]])
f_c <- c_ci_change(combine_imp(female_ars,'c_idx'),combine_imp(female_q,'c_idx'),c_shrink[[4]],c_shrink[[3]])
####################################################
source(file.path("risk_scores/",'reclass_scores.R'), echo = T)
source(file.path("risk_scores/",'reclass_sex.R'), echo = T)

path <- "risk_scores/array_job/master_branch/plot_scripts/outputs/reclass/"
png(paste0(path,'reclass_full.png'),type = 'cairo',width=1400, height=400)
count_reclass_score(df,base = 'q',pred = 'rs_full',lab = 'macce.incident',th = 10,res_path = path,name = "reclass_scores")
count_reclass(df,base = 'qrisk3_adj',pred = 'ars_adj',lab = 'macce.incident',th = 10,res_path = path,name = "reclass_q_ars")
count_reclass_sex(df,pred = 'qrisk3_adj',comp = 'ars_adj',lab = 'macce.incident',th = 10,res_path = path ,name = "reclass_q_ars_sex")
count_reclass_sex(df,pred = 'qrisk3_adj',comp = 'ars_con_adj',lab = 'macce.incident',th = 10,res_path = path ,name = "reclass_q_ars_con_sex")
dev.off()
...
pool_bs <- function(res,met,rows_with_na,M= 5, B = 1000){
  ...
  ci <- pool_ci(res_m)
  print(paste0(round(res_b_m,3),'(',round(ci[1],3),'-',round(ci[2],3),')'))
  return(c(ci[1],res_b_m,ci[2]))
}


pool_diff <- function(res1,res2,met1,met2,rows_with_na,M= 5, B = 1000,idx = 1){
  # Initialize an empty matrix to store res_b_m estimates
  ...
  #Theta_b_m point estimates for each boot strap
  #male_q_us
  res1_m <- remove_rows(res1,rows_with_na,met = met1) #sapply(res, function(df) df[-unique(rows_with_na),met])
  #res1_m <- sapply(res1, function(df) df[-unique(rows_with_na),met])
  res1_m <-rowMeans(res1_m)
  
  res2_m <- remove_rows(res2,rows_with_na,met = met2)
  ...
  print(paste0(round(mean_dif,3),'(',round(ci[1],3),'-',round(ci[2],3),')'))
...
female_ars_con = cond_res[[1]][[4]]
source(file.path("risk_scores",'cal_plot_imp.R'), echo = T)
full_path <- "risk_scores/ARS/bs_imp/con/6y/stacked/full/"
con_path <- "risk_scores/ARS/bs_imp/con/6y/stacked/condensed/"
cal_curve(df[sex_cond,],male_q,rows_with_na,lab = 'macce.incident', pred_event_prob = 'qrisk3_adj',xmin =0,xmax = 20,hist_max = 5000,title = "Male QRISK3 calibration sigmoid",store_path = paste0(full_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)
cal_curve(df[sex_cond,],male_ars,rows_with_na,lab = 'macce.incident', pred_event_prob = 'ars_adj',xmin =0,xmax = 20,hist_max = 5000,title = "Male ARS calibration sigmoid",store_path = paste0(full_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)

cal_curve(df[!sex_cond,],female_q,rows_with_na,lab = 'macce.incident', pred_event_prob = 'qrisk3_adj',xmin =0,xmax = 20,hist_max = 6500,title = "Female QRISK3 calibration ",store_path = paste0(full_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)
cal_curve(df[!sex_cond,],female_ars,rows_with_na,lab = 'macce.incident', pred_event_prob = 'ars_adj',xmin =0,xmax = 20,hist_max = 6500,title = "Female ARS calibration ",store_path = paste0(full_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)


cal_curve(df[sex_cond,],male_q_con,rows_with_na,lab = 'macce.incident', pred_event_prob = 'qrisk3_con_adj',xmin =0,xmax = 20,hist_max = 5000,title = "Male QRISK3 calibration sigmoid",store_path = paste0(con_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)
cal_curve(df[sex_cond,],male_ars_con,rows_with_na,lab = 'macce.incident', pred_event_prob = 'ars_con_adj',xmin =0,xmax = 20,hist_max = 5000,title = "Male ARS calibration sigmoid",store_path = paste0(con_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)

cal_curve(df[!sex_cond,],female_q_con,rows_with_na,lab = 'macce.incident', pred_event_prob = 'qrisk3_con_adj',xmin =0,xmax = 20,hist_max = 6500,title = "Female QRISK3 calibration ",store_path = paste0(con_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)
cal_curve(df[!sex_cond,],female_ars_con,rows_with_na,lab = 'macce.incident', pred_event_prob = 'ars_con_adj',xmin =0,xmax = 20,hist_max = 6500,title = "Female ARS calibration ",store_path = paste0(con_path,store,'/'),met_path = paste0('/gpfs3ecg_scores/macce_res/minb_set/comb/q_ecg/lin_res/mets/'),offset = 2.5)
...
m_mets <- list(male_q_con,male_ars_con,male_q,male_ars)
f_mets <- list(female_q_con,female_ars_con,female_q,female_ars)

for(met in f_mets){
  pool_bs(met,'c_idx',rows_with_na = rows_with_na)
}
