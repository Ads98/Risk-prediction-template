...
load_val_set<- function(split,folds,data_soruce,stacked = FALSE){
  ...
  #ecg_df <- read.csv("ecg_scores/macce_res/minb_set/ecg/full/full_agg.csv")
  ukbc <- read.csv("risk_scores/macce_all.csv")
  train<- read.csv("risk_scores/ARS/dataset_deriv_Sep2021.csv")
  deriv <- read.csv("risk_scores/ARS/dataset_test_Sep2021.csv")
  ...
  #ars_val_dat <- read.csv("risk_scores/ARS/dataset_test_Sep2021.csv")
  ...
  if(stacked){
    ...
    for(i in 1:10){
      cv_path <- paste0('risk_scores/ARS/nested_cv/split',i,'/')
      val_fold <- read.csv(paste0(cv_path,"predicted_test.csv"))
      ars_val_dat <- rbind(ars_val_dat)
    }
  }
  else{
    cv_path <- paste0('risk_scores/ARS/nested_cv/split',split,'/')
  ...
  epsilon <- 1e-6 
  ars_val_dat['SleepActivity'] <- (ars_val_dat$SleepActivity/100) *24
  ars_val_dat['ModerVigorActivity']<- ((ars_val_dat$ModerVigorActivity/100) *24) + epsilon 
  ars_val_dat['SedentaryActivity'] <- (ars_val_dat$SedentaryActivity/100) *24
  ars_val_dat['LightActivity'] <- (ars_val_dat$LightActivity/100) *24
  print('got 24hr PA')
